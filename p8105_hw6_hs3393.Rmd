...
The estimated odds ratio for solving homicides comparing male victims to victims is `r balt_result["victim_sexMale","OR"]`, the 95% confidence interval for it is (`r balt_result["victim_sexMale","lower"]`, `r balt_result["victim_sexMale","upper"]`). 
...
all_result %>% 
  ...
  labs(x = "Adjusted odds ratio", 
       y = "City, State",
       title = "Homicides solving odds ratio male vs female" )
```
...
data <- read_csv("data/birthweight.csv") %>% 
  mutate_at(c("frace", "malform", "mrace"), as.factor)
...
We will start model building by stepwise variable selection by AIC.
...
coef(models, id = 7)
```
Use adjusted R squared as the criteria, we can see that after the variable number greater than 7, the contribution of adding a new variavle will help adjuseted R squared increase less than 0.001, which is of trivial effect for the full model, so we use 7 as the cutoff point. 

```{r selected variable model}
select_model <- lm(bwt ~ bhead + blength + delwt + gaweeks + mrace + smoken + pnumsga, data = data)
```


```{r}
main_2_model <- lm(bwt ~ blength + gaweeks, data = data)

inter_3_model <- lm(bwt ~ bhead + blength + babysex + bhead:blength + bhead:babysex + blength:babysex + bhead:blength:babysex, data = data)
```
